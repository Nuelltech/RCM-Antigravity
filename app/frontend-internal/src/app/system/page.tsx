...
import { Database, Zap, Server, HardDrive, Activity, FileText, ShoppingBag } from "lucide-react";
...
function PerformanceTab() {
    ...
    useEffect(() => {
        const fetchMetrics = async () => {
            try {
                const data = await fetchWithAuth('/api/internal/health/performance?period=24h');
                setMetrics(data);
            } catch (error) {
...
function OverviewTab() {
    return (
        <div className="space-y-6">
            <SystemHealthCards />
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <StatCard title="System Uptime" value="99.8%" subtitle="Last 30 days" color="green" />
                <StatCard title="Avg Response Time" value="142ms" subtitle="Last hour" color="blue" />
                <StatCard title="Total Requests" value="1.2M" subtitle="Today" color="purple" />
            </div>
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
                <h3 className="font-semibold text-blue-900 mb-2">🚀 System Status</h3>
                <p className="text-blue-700 text-sm">All systems operational. Monitoring active and health checks passing.</p>
            </div>
        </div>
    );
}
...
function DatabaseTab() {
    return (
        <div className="bg-white rounded-xl p-8 border border-slate-200">
            <h2 className="text-xl font-bold mb-4">Database Metrics</h2>
            <div className="space-y-4">
                <MetricRow label="Status" value="Connected" status="success" />
                <MetricRow label="Response Time" value="42ms" />
                <MetricRow label="Active Connections" value="12 / 100" />
                <MetricRow label="Database Size" value="2.4 GB" />
                <MetricRow label="Last Backup" value="2 hours ago" status="success" />
            </div>
        </div>
    );
