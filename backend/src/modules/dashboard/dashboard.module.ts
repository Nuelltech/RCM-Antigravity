...
export async function dashboardRoutes(app: FastifyInstance) {
    app.withTypeProvider<ZodTypeProvider>().get('/stats', {
        ...
        const cacheKey = `tenant:${req.tenantId}:stats`;
        const cached = await dashboardCache.get(cacheKey);

        if (cached) {
            console.log(`[CACHE HIT] Dashboard stats for tenant ${req.tenantId}`);
            return reply.send(cached);
        }

        console.log(`[CACHE MISS] Calculating dashboard stats for tenant ${req.tenantId}`);
...
DATE(data_venda) as date,
    ...
    `;
        ...
        await dashboardCache.set(cacheKey, result, 300);
        console.log(`[CACHE SET] Dashboard stats cached for tenant ${req.tenantId}`);
