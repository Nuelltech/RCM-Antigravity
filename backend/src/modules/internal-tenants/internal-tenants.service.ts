...
export interface InvoiceFilters {
    status?: 'success' | 'error' | 'pending' | 'rejected' | 'approved' | 'reviewing' | 'imported';
    page: number;
    limit: number;
}
...
class InternalTenantsService {
    ...
    async getTenantInvoices(tenantId: number, filters: InvoiceFilters) {
        ...
        return {
            data: invoices.map(inv => ({
                ...inv,
                ficheiro_url: this.toPublicUrl(inv.ficheiro_url)
            })),
            pagination: {
                page,
                limit,
    ...
    async getInvoiceDetails(tenantId: number, invoiceId: number) {
        ...
        return {
            ...invoice,
            ficheiro_url: this.toPublicUrl(invoice.ficheiro_url)
        };
    }
    ...
    async getTenantSales(tenantId: number, filters: SalesFilters) {
        ...
        return {
            data: sales.map(sale => ({
                ...sale,
                ficheiro_url: this.toPublicUrl(sale.ficheiro_url)
            })),
            pagination: {
                page,
                limit,
    ...
    async getSalesDetails(tenantId: number, salesId: number) {
        ...
        return {
            ...sales,
            ficheiro_url: this.toPublicUrl(sales.ficheiro_url)
        };
    }
